...
<body class="antialiased bg-gray-900 text-white selection:bg-red-600 selection:text-white flex flex-col min-h-screen">
    ...
    <header class="w-full p-6 flex justify-between items-center z-10 border-b border-gray-800 bg-gray-900/80 backdrop-blur-md fixed top-0">
        <div class="flex items-center gap-3">
            <img src="{{ asset('logo.png') }}" alt="Logo Mirai Coffee" class="h-12 w-auto rounded-lg shadow-md border border-gray-700">
        ...
        @if (Route::has('login'))
            <div class="hidden sm:flex gap-4 items-center">
                @auth
                    <a href="{{ url('/home') }}" class="font-semibold text-gray-300 hover:text-red-500 transition px-4 py-2">Beranda Toko</a>
                @else
                    <a href="{{ route('login') }}" class="font-semibold text-gray-300 hover:text-red-500 transition px-4 py-2">Log in</a>

                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="font-semibold bg-[#E31E24] hover:bg-red-700 text-white px-5 py-2 rounded-lg transition shadow-lg border border-red-600">Register</a>
                    @endif
                @endauth
            </div>
        @endif
    ...
    <main class="flex-grow flex flex-col justify-center items-center text-center px-6 mt-20 relative">
        ...
        <div class="flex flex-col sm:flex-row gap-4 z-10">
            @auth
                <a href="{{ url('/home') }}" class="px-8 py-4 bg-[#E31E24] hover:bg-red-700 text-white font-bold rounded-full text-lg transition-transform hover:scale-105 shadow-[0_0_20px_rgba(227,30,36,0.4)]">
                    Lihat Menu Kami
                </a>
            @else
                <a href="{{ route('login') }}" class="px-8 py-4 bg-[#E31E24] hover:bg-red-700 text-white font-bold rounded-full text-lg transition-transform hover:scale-105 shadow-[0_0_20px_rgba(227,30,36,0.4)]">
                    Pesan Sekarang
                </a>
                <a href="{{ route('register') }}" class="px-8 py-4 bg-transparent hover:bg-yellow-400/10 border-2 border-yellow-400 text-yellow-400 font-bold rounded-full text-lg transition">
                    Daftar Member
                </a>
            @endauth
        </div>
    ...
    <footer class="w-full text-center py-6 text-sm text-gray-500 border-t border-gray-800 mt-auto z-10">
        &copy; {{ date('Y') }} Mirai Coffee & Snack. All rights reserved.
    </footer>
